...
class GridCoords(ABC):
    ...
    in a 3D scene. It does not map with an actual coordinate system used in
    Earth Observation or to a specific projection explicitely.

    Two coordinate systems are used internally, one for spherical shell atmospheric geometries and
    another for plane parallel geometries.
...
class PlaneParallelGridCoords(GridCoords):
    ...
    def from_extent_and_resolution(
        ...
        PlaneParallelGridCoords
        """
        extent_x = ensure_units(extent_x, ucc.get("length"))
        extent_y = ensure_units(extent_y, ucc.get("length"))
    ...
    def from_extent_and_cell_size(
        ...
        extent_x = ensure_units(extent_x, ucc.get("length"))
        extent_y = ensure_units(extent_y, ucc.get("length"))
        cell_width = ensure_units(cell_width, ucc.get("length"))
        cell_length = ensure_units(cell_length, ucc.get("length"))
        if atol is not None:
            atol = ensure_units(atol, ucc.get("length"))
