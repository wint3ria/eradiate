...
def __thermoprops_grid():
    return [
        MolecularAtmosphere().thermoprops for _ in range(9)
    ]  # TODO dedicated 3D gridded type
    ...
    [
        lambda: dict(
            molecular_atmosphere=GriddedMolecularAtmosphere(
                thermoprops_grid=__thermoprops_grid()
            )
        ),
        lambda: dict(molecular_atmosphere=None, particle_layers=ParticleLayer()),
    ],
